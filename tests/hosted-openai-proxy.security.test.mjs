...
test("hosted OpenAI access keeps the shared key behind a bounded server proxy", async () => {
  ...
  assert.match(browserWorker, /const proxy = await pyodide\.runPythonAsync\(`/);
  assert.match(router, /output = run_sync\(\s*Runner\.run\(/);
  ...
  assert.match(router, /response_headers = \{\s*"content-type": response\.headers\.get\(/);
  assert.doesNotMatch(router, /headers=response\.headers/);
